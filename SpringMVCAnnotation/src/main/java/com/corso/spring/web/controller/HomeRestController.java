...
//@RequestMapping(path={"/", "/altro"}) // si possono impostare più path
public class HomeRestController {
	...
	public String provaProduces2 () {
		return "{message:\"Contenuto Json in risposta  alla richiesta!\"}";
	}			//@ResponseBody
	@RequestMapping(
			path={"/headers", "/produces"},
			method= {RequestMethod.GET, RequestMethod.POST},
			params = {"nome=Mario", "cognome=Verdi"},
			//headers = {"Accept=application/json"},
			//produces = {"application/json"}
			produces  = {MediaType.APPLICATION_JSON_VALUE} 
	)
	public String provaProduces () {
		return "{message:\"Contenuto Json in risposta  alla richiesta! 2\"}";
	}
	...
	@ResponseBody
	@GetMapping(path={"/rettangolo"}, 
	produces = {"application/json"})
	 
	public Rettangolo provaBean () {
	    
		return new Rettangolo(4,5);
	}
